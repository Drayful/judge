            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <x-card>
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        @if(Auth::user()->isSecretary() || Auth::user()->isChiefJudge() || Auth::user()->isAdmin())
                            <a class="text-emerald-400 hover:text-emerald-300 font-medium" href="{{ route('secretary.tournaments') }}">{{ Auth::user()->isChiefJudge() ? 'Главный судья' : 'Секретарь' }} · Турниры и протоколы</a>
                            <a class="text-emerald-400 hover:text-emerald-300 font-medium" href="{{ route('secretary.categories') }}">{{ Auth::user()->isChiefJudge() ? 'Главный судья' : 'Секретарь' }} · Категории / Live</a>
                        @endif
