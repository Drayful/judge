                ...
                <div class="flex items-center justify-between gap-3 mb-4">
                    <div>
                        ...
                        <div class="text-sm text-slate-400 mt-1">
                            Скачивание результатов по году рождения и категории (формат как в образце results_*.xlsx).
                        </div>
                    </div>
                    <x-badge tone="violet">{{ ($protocolGroups ?? collect())->count() }} групп</x-badge>
                </div>

                @if(($protocolGroups ?? collect())->isEmpty())
                    <div class="text-sm text-slate-400">
                        Пока нет данных для протоколов. Импортируйте стартовый протокол и проведите судейство —
                        здесь появятся группы (например «2015 г.р. — категория A») с кнопкой скачивания.
                    </div>
                @else
                        ...
                        @foreach($protocolGroups as $g)
                            <div class="border border-slate-800 rounded-xl p-4 bg-slate-950/40 flex items-center justify-between gap-3">
                                <div class="min-w-0">
                                    ...
                                    <div class="text-xs text-slate-500 mt-1">{{ $g['athletes'] }} гимнасток с результатом</div>
                                </div>
                                <a class="shrink-0 inline-flex items-center gap-1.5 rounded-lg border border-emerald-700/60 bg-emerald-950/40 px-3 py-2 text-xs font-semibold text-emerald-100 hover:bg-emerald-900/60 hover:border-emerald-600 transition {{ $g['athletes'] === 0 ? 'pointer-events-none opacity-40' : '' }}"
                                   href="{{ route('secretary.tournament.protocol', $tr) }}?birth_year={{ $g['birth_year'] }}&division={{ $g['division'] }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
