...
class LiveResultWorkflowTest extends TestCase
{
    ...
    public function test_group_tablets_expose_the_requested_db_and_artistry_rules(): void
    {
        ...
        $this->actingAs($aJudge)
            ...
            ->assertSee('Контраст')
            ->assertSee('Быстрая последовательность / канон')
            ->assertSee('Хоровая работа')
            ->assertSee('не отмечено = −0.30')
            ->assertSee('text-3xl leading-none text-emerald-200', false)
            ->assertDontSee('Убрать')
            ->assertSee('Достаточно рисунков построений')
            ->assertSee('Достаточная амплитуда построений')
    ...
    public function test_individual_a_tablet_contains_every_fig_artistry_penalty(): void
    {
        ...
        $this->actingAs($judge)
            ...
            ->assertSee('Ритм')
            ->assertSee('Площадка использована достаточно')
            ->assertSee('Нет прерывания непрерывности 4+ сек.')
            ->assertSee('Музыкальное вступление короче 4 сек.')
            ->assertSee('Музыка соответствует нормам')
            ->assertSee('Окончание совпадает с музыкой')
            ->assertSee('Экспрессия лица достаточная')
